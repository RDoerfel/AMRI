function [imageSense, g_map] = sense(CoilImages, sensitivityMaps, noiseCorr, R, nYCal, nXCal, nYPI)

shifted_CoilImages = circshift(CoilImages, -floor(nYPI/2), 1);
shifted_sensitivityMaps = circshift(sensitivityMaps, -floor(nYCal/2), 1);
...
for y = 1:nYPI
    ...
    for x = 1:nXCal
        ...
        % If no noise correlation matrix is provided -> assume identity matrix:
