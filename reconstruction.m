...
% load('TSE_8chheadcyl', 'M', 'I'); % seqType = -2;
% load('InterleavedEPI_8ch_Cal.mat', 'M', 'I'); seqType = 3;

load('spinEcho_EPI_8ch_CAL_10T2_0CS_0T2star.mat', 'M', 'I'); seqType = 1; % Normal EPI
...
[~, CoilImages] = getMatrices(Mxy_cal, nXCal, nYCal, nCoils, seqType);
CoilImages = imresize(CoilImages,[nYCal*2, nXCal]);
...
maskCalibration = imresize(maskCalibration,[nYCal*2, nXCal]);
...
imageReconstructed = imresize(imageReconstructed,[nYCal*2, nXCal]);
%
...
% load('eight_InterleavedEPI_8ch_R2.mat', 'M', 'I'); seqType = 5; % 8-Interleaved EPI
load('spinEcho_EPI_8ch_R4_10T2_0CS_0T2star.mat', 'M', 'I'); seqType = 1; % Normal EPI
...
% Get the reduced-FOV images from all coils:
[~, CoilImages_PI] = getMatrices(Mxy_PI, nXPI, nYPI, nCoils, seqType);
...
%%
clc
[SENSEreconstruction, g_map] = sense(CoilImages_PI, sensitivityMaps, [], nXCal, nXCal, nYPI, nXPI);

SENSEreconstruction = SENSEreconstruction(~all(SENSEreconstruction == 0, 2),:);
g_map = g_map(~all(g_map == 0, 2),:);
new_maskCalibration = maskCalibration(~all(SENSEreconstruction == 0, 2),:);


figure('WindowState', 'maximized')
imagesc((abs(SENSEreconstruction) .* new_maskCalibration))
colormap('gray');colorbar; axis image; title(['SENSE-reconstructed image. R=',num2str(nXCal/nYPI)])
%
figure
imagesc(abs(g_map).*new_maskCalibration)
colormap('gray');colorbar; axis image; title(['g-map. R=', num2str(nXCal/nYPI)])
